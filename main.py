...
app.config['MAIL_USERNAME'] = 'adaptivewavesinc@gmail.com'  # Your Gmail address
app.config['MAIL_PASSWORD'] = 'abez njbq iqjo janw'
...
def send_otp():
    ...
    if email in otp_storage and time.time() < otp_storage[email]['expires'] - 300 + 60:
        return jsonify({"error": "Please wait a minute before requesting another OTP."}), 429
...
def delete_material():
    ...
    if not folder or not filename:
        return jsonify({"success": False, "error": "Folder or filename not provided."}), 400
...
def send_feedback():
    ...
    try:
        ...
        return jsonify({"status": "error", "message": "Failed to send feedback. Please try again."})
